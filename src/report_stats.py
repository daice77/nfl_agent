...
weekly_roster = import_and_clean_data(nfl.import_weekly_rosters, "Weekly Roster", [YEAR])
...
# Snap count data
snap_columns = [
    "pfr_player_id", "team", "week", "offense_snaps", "offense_pct", 
    "defense_snaps", "defense_pct", "st_snaps", "st_pct"
]
snap_counts = import_and_clean_data(nfl.import_snap_counts, "Snaps", [YEAR])[snap_columns]
...
team_player_data = pd.merge(
    team_player_data, 
    weekly_roster[['player_id', 'first_name', 'last_name', 'team', 'position', 'pfr_id', 'week']], 
    on=['player_id', 'week'], 
    how='left'
)

# Step 4: Aggregating weekly and snap data by player for the season
aggregated_data = team_player_data.groupby(['player_id', 'player_name', 'position_x', 'recent_team']).agg({
    'completions': 'sum',
    'attempts': 'sum',
    'passing_yards': 'sum',
    'passing_tds': 'sum',
...
key_columns = [
    "player_name", "position_x", "recent_team", "weeks_played",
...
def team_roster(data, team_name):
    ...
    for _, row in team_data.iterrows():
        ...
        print(f"  Passing: {row['completions']} completions, {row['passing_yards']} yards, {row['passing_tds']} TDs")
        print(f"  Rushing: {row['carries']} carries, {row['rushing_yards']} yards, {row['rushing_tds']} TDs")
        print(f"  Receiving: {row['receptions']} receptions, {row['receiving_yards']} yards, {row['receiving_tds']} TDs")
        print(f"  Fantasy Points: {row['fantasy_points_ppr']} PPR")
        print(f"  Offensive Snaps: {row['offense_snaps']} | Offensive Snap Percentage: {row['offense_pct']}%")
        print(f"  Defensive Snaps: {row['defense_snaps']} | Defensive Snap Percentage: {row['defense_pct']}%")
        print(f"  Special Teams Snaps: {row['st_snaps']} | Special Teams Snap Percentage: {row['st_pct']}%")
        print("-" * 40)
    print("\n")
